...
func randomHexColor() string {
	// Generate three random 8-bit integers for Red, Green, and Blue.
	red := rand.Int32N(256)
	green := rand.Int32N(256)
	blue := rand.Int32N(256)

	// Format the integers as a hex string.
	// The "%02x" format specifier ensures that each number is represented by
	// at least two hexadecimal digits, with a leading zero if necessary.
