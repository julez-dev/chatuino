...
// SuggestionTextInput is the Bubble Tea model for this text input element.
// It is a copy of bubbles text input https://github.com/charmbracelet/bubbles/blob/167e9067bc4eb2566334f33787cdfdf1bcc25124/textinput/textinput.go
// However, it has been modified to support multiple suggestions, even when the cursor is inside the input.
...
func (m *SuggestionTextInput) SetValue(s string) {
...
func (m *SuggestionTextInput) setValueInternal(runes []rune) {
	if m.Validate != nil {
...
func (m SuggestionTextInput) Value() string {
	return string(m.value)
}

// Position returns the cursor position.
func (m SuggestionTextInput) Position() int {
	return m.pos
}

// SetCursor moves the cursor to the given position. If the position is
// out of bounds the cursor will be moved to the start or end accordingly.
func (m *SuggestionTextInput) SetCursor(pos int) {
	m.pos = clamp(pos, 0, len(m.value))
	m.handleOverflow()
}

// CursorStart moves the cursor to the start of the input field.
func (m *SuggestionTextInput) CursorStart() {
	m.SetCursor(0)
}

// CursorEnd moves the cursor to the end of the input field.
func (m *SuggestionTextInput) CursorEnd() {
	m.SetCursor(len(m.value))
}

// Focused returns the focus state on the model.
func (m SuggestionTextInput) Focused() bool {
	return m.focus
}

// Focus sets the focus state on the model. When the model is in focus it can
// receive keyboard input and the cursor will be shown.
func (m *SuggestionTextInput) Focus() tea.Cmd {
	m.focus = true
...
func (m *SuggestionTextInput) Blur() {
	m.focus = false
	m.Cursor.Blur()
}

// Reset sets the input to its default state with no input.
func (m *SuggestionTextInput) Reset() {
	m.value = nil
	m.SetCursor(0)
}

// SetSuggestions sets the suggestions for the input.
func (m *SuggestionTextInput) SetSuggestions(suggestions []string) {
	m.suggestions = [][]rune{}
...
func (m *SuggestionTextInput) san() runeutil.Sanitizer {
	if m.rsan == nil {
...
func (m *SuggestionTextInput) insertRunesFromUserInput(v []rune) {
	// Clean up any special characters in the input provided by the
...
func (m *SuggestionTextInput) handleOverflow() {
...
func (m *SuggestionTextInput) deleteBeforeCursor() {
...
// masked input.
func (m *SuggestionTextInput) deleteAfterCursor() {
	m.value = m.value[:m.pos]
	m.SetCursor(len(m.value))
}

// deleteWordBackward deletes the word left to the cursor.
func (m *SuggestionTextInput) deleteWordBackward() {
...
// delete everything after the cursor so as not to reveal word breaks in the
// masked input.
func (m *SuggestionTextInput) deleteWordForward() {
...
func (m *SuggestionTextInput) wordBackward() {
	if m.pos == 0 || len(m.value) == 0 {
		return
	}
...
func (m *SuggestionTextInput) wordForward() {
	if m.pos >= len(m.value) || len(m.value) == 0 {
		return
	}
...
func (m SuggestionTextInput) echoTransform(v string) string {
	switch m.EchoMode {
	case EchoPassword:
...
func (m SuggestionTextInput) Update(msg tea.Msg) (SuggestionTextInput, tea.Cmd) {
	if !m.focus {
		return m, nil
	}
	...
	if ok && key.Matches(keyMsg, m.KeyMap.AcceptSuggestion) {
		if m.canAcceptSuggestion() {
			word, startWord, endWord := selectWordAtIndex(string(m.value), m.pos)
			remainingRunes := m.matchedSuggestions[m.currentSuggestionIndex][len(word):]

			m.value = slices.Insert(m.value, startWord+len(word), remainingRunes...)
			m.SetCursor(endWord + len(remainingRunes))
			// m.CursorEnd()
		}
	}
...
func (m SuggestionTextInput) View() string {
	// Placeholder text
	...
	if pos < len(value) {
		if m.canAcceptSuggestion() {
			...
			word, start, end := selectWordAtIndex(string(value), pos)
			if len([]rune(word)) < len(suggestion) {
				pos = pos - start
				...
				v += " "
				v += styleText(m.echoTransform(string(value[end+1:])))
			} else {
		...
		if m.canAcceptSuggestion() {
			suggestion := m.matchedSuggestions[m.currentSuggestionIndex]
			word, start, _ := selectWordAtIndex(string(value), pos)
			if len([]rune(word)) < len(suggestion) {
				pos = pos - start
				m.Cursor.TextStyle = m.CompletionStyle
				m.Cursor.SetChar(m.echoTransform(string(suggestion[pos])))
...
func (m SuggestionTextInput) placeholderView() string {
	var (
		v     string
...
func (m SuggestionTextInput) completionView(offset int) string {
	var (
		value = m.value
		style = m.PlaceholderStyle.Inline(true).Render
	)

	word, _, _ := selectWordAtIndex(string(value), m.pos)
	wordRunes := []rune(word)

	if m.canAcceptSuggestion() {
		suggestion := m.matchedSuggestions[m.currentSuggestionIndex]
		if len(wordRunes) < len(suggestion) {
			return style(string(suggestion[len(wordRunes)+offset:]))
		}
	}
	return ""
}
...
func (m *SuggestionTextInput) AvailableSuggestions() []string {
	suggestions := []string{}
...
func (m *SuggestionTextInput) CurrentSuggestion() string {
...
func (m *SuggestionTextInput) canAcceptSuggestion() bool {
	return len(m.matchedSuggestions) > 0
}

// updateSuggestions refreshes the list of matching suggestions.
func (m *SuggestionTextInput) updateSuggestions() {
	...
	matches := [][]rune{}
	word, _, _ := selectWordAtIndex(strings.ToLower(string(m.value)), m.pos)

	for _, s := range m.suggestions {
		suggestion := string(s)

		if strings.HasPrefix(strings.ToLower(suggestion), word) {
...
func (m *SuggestionTextInput) nextSuggestion() {
...
func (m *SuggestionTextInput) previousSuggestion() {
	m.currentSuggestionIndex = (m.currentSuggestionIndex - 1)
