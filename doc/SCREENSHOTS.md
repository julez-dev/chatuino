# Screenshots

## Chat View
![Chat View](screenshot/chat-view.png)

## Vertical Mode
![Vertical Mode](screenshot/vertical-mode.png)

## Account Management
![Account UI](screenshot/account-ui.png)

## New Window Prompt
![New Window Prompt](screenshot/new-window-prompt.png)

## Message Search
![Message Search](screenshot/message-search.png)

## Message Log
![Message Log](screenshot/message-log.png)

## Cache Statistics
![Cache Statistics](screenshot/cache-stats.png)

### Emote Completions
![Emote Completions](screenshot/auto-completions-emotes.png)

### User Completions
![User Completions](screenshot/auto-completions_user.png)
